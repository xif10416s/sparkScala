...
import org.fxi.test.spark.bean.UserCreditLogDaily
 ...
 */
class UserCreditLogDailyBaseInfoSchemaLoader extends SchemaLoader {
  ...
  override def loadSchema(ctx: SparkContext, sqlCtx: SQLContext): Unit = {
    ...
    for (path <- paths) {
      println(path);
      val creditLogDailyRdd = ctx.textFile(path).map(_.split(Utils.SPLIT_TAB)).map(u => UserCreditLogDaily(u(0),null, null,
        Utils.parseToInt(u(1)), null ,Utils.getCreditObtainTime(u(4),u(5)), null));
    ...
    schemaPeople.registerTempTable("creditLogDailyBaseInfo");
    schemaPeople.cache();
  }
